<?php
// php-mf2 <microformats/php-mf2>, CC0.
 ...
 *
 *     use microformats;
 *     $output = microformats\parse('<span class="h-card">Barnaby Walters</span>');
 ...
 *
 *     use microformats;
 *     $parser = new microformats\Parser('<p class="h-card">Barnaby Walters</p>');
 *     $output = $parser->parse();
 */
class Parser {
	...
	public function __construct($input, $url = null, $jsonMode = false) {
		...
		$this->xpath->registerPhpFunctions('\\microformats\\classHasMf2RootClassname');
	...
	public function getRootMF(DOMElement $context = null) {
		...
		$xpaths = array(
			'(php:function("\\microformats\\classHasMf2RootClassname", normalize-space(@class)))'
		);
